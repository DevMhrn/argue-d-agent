"""Deterministic canned model outputs (ported from src/mockResponses.ts).

Each key matches a `mock_key` passed by the pipeline so the whole thing runs with
no keys and no network. advocate_position#1 deliberately leaves one point uncited
so the Citation Gate visibly rejects it and the agent fixes it on #2.
"""
