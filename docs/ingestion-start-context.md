...
Historical note: this section was written when eight agents were routed across Anthropic / Google / OpenAI. Current active defaults use Anthropic + OpenAI; Gemini remains configurable only if a key is available. Sections below that still mention Gemini as active are historical.

| # | Agent | Provider | Job |
|---|-------|----------|-----|
| 1 | Intake Parser | OpenAI (gpt-4o-mini) | Extract incident facts from the First Notice of Loss |
| 2 | Evidence Aggregator | OpenAI | Build the structured Evidence Ledger from documents |
...
| 6 | Adjudicator B | OpenAI | Independent adjudicator on a different family (consensus check) |
| 7 | Source-Alignment Verifier | Anthropic | Audit every cited claim against its source fact |
| 8 | Demand Letter Drafter | Anthropic (claude-sonnet-4-6) | Compose the formal demand letter |

**The Advocate (Claude) debates the Opposing red team (GPT); Adjudicator A (Claude) is checked against Adjudicator B (GPT)** — different families across the debate, with an independent adjudicator pair for the consensus check.
...
7. **`transcript`** — per-pipeline-run room postings with structured metadata. `run_id` generated by the orchestrator at run start; `(run_id, seq)` is canonical ordering.
...
- `last_run_at TIMESTAMPTZ NULL` — denormalized rollup of the most recent run's timestamp. Reserved until the orchestrator updates it reliably. UI sorts cases by recency without joining to runs. Index `cases_last_run_at_idx (last_run_at DESC NULLS LAST)`.
