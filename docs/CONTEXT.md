...
- `backend/ledger/` — **Gowtham's lane**: `graph.py` (typed nodes/edges + validation + render), `builder.py` (`build_ledger` + `graph_to_evidence_ledger`), `prompts.py`, `mock_graphs.py`, `repository.py` (Supabase writes + dry_run), `build_demo.py`.
