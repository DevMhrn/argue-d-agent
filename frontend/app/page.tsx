...
export default async function CasesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-6 py-10">
      <div className="flex items-end justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted">
            ...
            the recovery argument, the opposing red team attacks it, two adjudicators
            on different model families decide, and a verifier audits every cited claim.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/cases/new"
            className="rounded-[9px] border border-border bg-accent/15 px-4 py-2 text-sm text-accent hover:bg-accent/25"
          >
            + New case
          </Link>
        </div>
      ...
      {backendOffline ? (
        <div className="mt-10 rounded-[14px] border border-bad/40 bg-bad/5 p-6 text-sm text-muted">
        ...
        <div className="mt-10 space-y-10">
          ...
          <section>
            <header className="mb-3 flex items-baseline justify-between">
              <h2 className="text-base font-semibold text-text">Your cases</h2>
              <span className="text-[12px] text-muted-2">
                {dbCases.length} stored in Supabase
              </span>
            </header>
            {dbError ? (
              <div className="rounded-[14px] border border-bad/40 bg-bad/5 p-4 text-[13px] text-bad">
                Couldn&apos;t reach Supabase: {dbError}
              </div>
            ) : dbCases.length === 0 ? (
              <div className="rounded-[14px] border border-border bg-panel p-8 text-center text-sm text-muted">
                <p>No cases yet.</p>
                ...
                {dbCases.map((c) => (
                  <li key={c.id}>
                    <Link
                      ...
                      className="flex items-start justify-between gap-6 rounded-[14px] border border-border bg-panel p-5 shadow-card transition-colors hover:border-accent"
                    >
                      <div className="min-w-0 flex-1">
                        <div className="font-medium">{c.title || c.case_id}</div>
                      ...
                      <span
                        className={`inline-flex shrink-0 items-center rounded-full border px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-wider ${STAGE_TONE[c.stage]}`}
                      >
                        {STAGE_LABEL[c.stage]}
                      </span>
                    </Link>
          ...
          <section>
            <header className="mb-3 flex items-baseline justify-between">
              <h2 className="text-base font-semibold text-text">Demo cases</h2>
              <span className="text-[12px] text-muted-2">
                ...
                <li key={c.id}>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      {c.subtitle ? (
                        <div className="mt-1 text-[13px] text-muted">{c.subtitle}</div>
                      ) : null}
                      <div className="mt-2 font-mono text-[11px] text-muted-2">{c.id}</div>
                    </div>
                    {c.outcome ? (
                      <span
                        className={`inline-flex shrink-0 items-center rounded-full border px-2.5 py-0.5 text-[11px] font-medium uppercase tracking-wider ${
                          c.outcome === "decline"
                            ? "border-bad/40 bg-bad/10 text-bad"
